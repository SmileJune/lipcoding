...
- 필요한 **MCP·도구는 자율적으로 설치·사용**해도 됩니다(매번 확인 불필요). 설치 후 사용하고 결과만 보고.
...
## 완료 알림 (Telegram)
- 작업(빌드·테스트·배포 등)이 끝나면 `node scripts/codex-notify.mjs done "<한 줄 요약>"` 으로 텔레그램 알림을 보낸다. 실패 시 상태를 `failed` 로.
- 자격증명은 `.env.codex-notify`(gitignore) 에 보관. 템플릿은 `.env.codex-notify.example`. 토큰은 출력·커밋 금지.
